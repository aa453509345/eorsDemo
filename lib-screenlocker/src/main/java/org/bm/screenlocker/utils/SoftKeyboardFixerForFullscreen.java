...
//        isNavigationShowing = ((decorView.getSystemUiVisibility() & View
// .SYSTEM_UI_FLAG_HIDE_NAVIGATION) == 0);
//        isFullscreenMode = ((decorView.getSystemUiVisibility() & View
// .SYSTEM_UI_FLAG_FULLSCREEN) == 0);//api 16以上
//        decorView.setOnSystemUiVisibilityChangeListener(new View
// .OnSystemUiVisibilityChangeListener() {//参考：https://blog.csdn.net/auccy/article/details/80632429
//            @Override
...
//        });
        ...
        mContentView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver
            ...
            public void onGlobalLayout() {//软键盘弹出、系统导航栏隐藏显示均会触发这里
                ...
                if (usableHeight != lastUsableHeight) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && activity
                            .isInMultiWindowMode()) {//如果是分屏模式
                        if (heightDifference > 0)
                        {//分屏模式，只要变动了就人为弹出键盘，因为分屏可能该Activity是在手机屏幕的上方，弹出输入法只是遮盖了一丁点~如果不合适，需要你自己适配了！
                            setHeight(heightDecor - heightDifference, mDelay); //这里不能加状态栏高度哟~
                        } else {
                            setHeight(FrameLayout.LayoutParams.MATCH_PARENT, 0);
                        ...
                        {//高度变动超过decor的四分之一则认为是软键盘弹出事件，为什么不用屏幕高度呢？开始以为这样在分屏模式下也可以监听，但是实测不行。
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                                setHeight(heightDecor - heightDifference + barStatusHeight, mDelay);
                                //这里为什么要添加状态栏高度？
                            } else {
                                setHeight(heightDecor - heightDifference, mDelay);
    ...
    private void setHeight(final int height, long delay) {
    ...
    //下面相关代码来自：https://github.com/yy1300326388/AndroidBarUtils/blob/master/app/src/main/java/cn
    // /zsl/androidbarutils/utils/AndroidBarUtils.java
    //完整代码，全屏时有问题。
